...
public %>% 
  ...
  mutate(q15_fee_prevent_filing = recode_factor(q15_fee_prevent_filing,
                                                ...
                                                `0` = "לא",
                                                `-99` = "לא יודע")) %>% 
  ggplot_factor(q15_fee_prevent_filing, 
                gtitle = str_wrap("כפי שנאמר בשאלה הקודמת, סכום האגרה בבית משפט מחוזי הוא 16,000 שח. האם סכום זה ימנע ממך להגיש תביעה יצוגית?", width = 60) %>% 
                  str_replace_all(., "\n", "\n\u202B")) -> prevent_filing_chart
...
profitable_fields_chart <- attorney %>% 
  ...
  arrange(desc(prop)) %>% 
  ggplot_factor(item) + 
  labs(caption = paste0("n=", total_sample, ". total percent > 100% because more than one selection was possible")) +
...
likert_scale_recoded <- attorney %>% 
  ...
  mutate(item = recode(item,
                       q2_new_regulations = "צמצום האפשרויות להגשת תביעות נגד המדינה ורשויות סמך - באיזו מידה ההתפתחות חיובית",
                       q3_supreme_court_pro_class = "באיזו מידה בית המשפט העליון אוהד את תחום התובענות הייצוגיות",
                       q4_legal_fees = "שכר הטרחה משקף את מאמץ עורך הדין בתביעה",
                       q5_file_dismissal_legal_fees = "בהסתלקויות יש הצדקה לשכ''ט וגמול",
                       q6_class_amount = "ב-2018 הוגשו כ-1,200 תובענות. באיזו מידה כמות זו ראויה?",
                       q7_attorney_general_position = "עמדות היועמ''ש בתובענות ייצוגיות חשובות",
                       q8_defend_public = "תובענות ייצוגיות עוזרות להגן על זכויות הציבור") %>% 
           str_wrap(width = 40) %>% 
           str_replace_all("\n", "\n\u202B")) %>% 
...
likert_scale_recoded_all <- likert_scale_recoded %>% 
  ...
  mutate(ranking_factor = fct_inorder(ranking_factor)) %>% 
  filter(q18_representation %in% c("prosecutors", "defendents")) %>%
  ungroup() %>% 
...
positions_split <- ggplot(likert_scale_recoded_all, aes(x = item, y = prop, fill = ranking_factor)) +
  geom_bar(stat = "identity", position = "fill", color = "black") +
  geom_label(aes(label = paste0(round(prop*100), "%")),position = position_fill(),
             show.legend = F) +
  guides(fill = guide_legend("דירוג")) +
